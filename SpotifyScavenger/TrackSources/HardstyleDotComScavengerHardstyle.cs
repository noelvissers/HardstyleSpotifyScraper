...
{
  public class HardstyleDotComScavengerHardstyle : IScavenge
  {
    public List<TrackData> ScavengeForTracks()
    {
      ...
      HtmlDocument documentAlbums = web.Load(@"https://music.hardstyle.com/hardstyle-releases/albums");
      IEnumerable<HtmlNode> nodes2 =
              documentAlbums.DocumentNode.Descendants("td")
                  .Where(n => n.HasClass("text-1"));

      int maxAmountOfAlbums = 15;
        ...
        foreach (var node2 in listOfNodes2)
        {
          ...
          foreach (var htmlNode in node2.ChildNodes)
          {
            ...
            if (htmlNode.Name == "b" && nodeElement % 2 == 1)
            {
              ...
              Console.Write($"[NEW] {htmlNode.InnerHtml}");
            }
      ...
      for (int i = 1; i < 2; i++)
      {
        ...
        HtmlDocument document = web.Load(@"https://music.hardstyle.com/hardstyle-releases/tracks/page/" + i.ToString());
